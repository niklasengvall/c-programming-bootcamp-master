 ...
 * 4 | 70% - 80% | moderate, aerobic fitness, improve your effiency
...
void print_table(Zone zones[], int len, double hr_max)
{
    ...
    {
        double upper = z < (zones + len - 1) ? (z + 1)->low * hr_max : hr_max; // Get the next zones lower threshold as current zones upper threshold
...
{
    for (int i = zones_len - 1; i >= 0; i--)    // Start from highest threshold
    {
        if (hr_now > zones[i].low * hr_max)
        {
            return &zones[i];   // Return zones address
        }
    }
    return NULL;                // For saftey    
}

void print_zone(Zone *cur_zone)
{
    printf("Zone: %d, %s\n\n", cur_zone->id, cur_zone->str); 
}

int main()
{
    Zone zones[] = {
        {.id = 1, .low = 0.0, .str = "rest"},
        {.id = 2, .low = 0.5, .str = "very light, active recovery"},  
        {.id = 3, .low = 0.6, .str = "light, cardio training, improves your general endurance"},  
        {.id = 4, .low = 0.7, .str = "moderate, aerobic fitness, improve your effiency"},  
    ...
    int zones_len = sizeof(zones) / sizeof(zones[0]); // Determine number of zones 
    int age;
    ...
    while (true)
    {
        ...
        if (hr_now <= 0)    // break if user enters zero or below
        {
            break;
        }
